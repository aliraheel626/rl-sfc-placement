...
\begin{enumerate}
  ...
  \item A \textbf{security cost model} that quantifies operational risk as the mean per-node product of incident probability (driven by security posture, load, and incident pressure), chain co-location count, and TTL-proportional exposure, as a deterministic security cost heuristic coupled to the reward signal.
...
The security cost model introduces a principled operational objective beyond simple acceptance ratio maximization. By coupling the deterministic security cost heuristic to the reward signal, the intelligent DRL agent learns to prefer placements that are both feasible and operationally resilient, avoiding nodes with degraded security posture and high co-location counts. The incident pressure decay model captures the temporal dynamics of node security posture in a shared NFV environment.
...
Table~\ref{tab:conclusion_compare} summarizes the mean metrics across evaluation episodes for Best-Fit versus Maskable PPO on the same aligned request stream (RNG state matched between algorithms). Acceptance ratio and total revenue differ by less than one percent in favour of Best-Fit, but PPO substantially improves security-oriented outcomes: lower average security cost heuristic and higher average security margin.
