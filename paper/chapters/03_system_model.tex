...
\end{equation}
where $R_{\text{acc}} = 1$, $\text{SFC}^{-}(n_{K_r})$ is the chain count on the last placement node \emph{before} the current SFC is registered, $\omega = 0.5$, and $\overline{\text{AR}}_w$ is the acceptance ratio over the last $w = 10$ requests.
...
\subsection{Per-Node Risk Score}

The per-node base incident probability $q_n$ drives the security-aware features observed by the GNN policy and the node selection criterion of the Best-Fit baseline:
...
The per-node risk score used by the Best-Fit baseline accounts for co-location and TTL exposure:
\begin{equation}
\rho_n^{\text{node}} = \min\!\left(q_n \cdot \text{SFC}(n) \cdot \frac{T_s}{\tau_{\max}},\; 1\right),
\label{eq:node_risk}
\end{equation}
where $T_s = \min(\tau_r, T_{\text{cap}})$ with $T_{\text{cap}} = 12$ and $\text{SFC}(n)$ is the number of active chains on node $n$.
