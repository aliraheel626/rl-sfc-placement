...
Avg Security Cost Heuristic & 0.0014 & 0.0006 & $-59.45$\% & PPO \\
...
Avg SFC Tenancy (SFCs/occupied node) & --- & --- & --- & --- \\
Avg VNF Tenancy (VNFs/occupied node) & --- & --- & --- & --- \\
\hline
\end{tabularx}
\end{table}
...
\subsection{Security Cost Heuristic During Training}

Figure~\ref{fig:risk} shows the evolution of the average security cost heuristic $\rho_r$ over training episodes. As the agent becomes more adept at minimizing risk-aware placement, the average security cost per accepted SFC decreases.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.8\textwidth]{models/sfc_risk_training.png}
\caption{Average security cost heuristic per accepted SFC during training.}
\label{fig:risk}
\end{figure}
...
Figure~\ref{fig:risk_compare} shows the rolling average security cost heuristic for both algorithms. The risk penalty in the RL reward function incentivizes the agent to prefer low-risk placements, which may trade some acceptance ratio for reduced operational risk relative to a greedy risk-only baseline.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.8\textwidth]{compare/sfc_risk_training.png}
\caption{Rolling average security cost heuristic (window = 50) for Maskable PPO and Best-Fit over 1000 requests.}
...
Figure~\ref{fig:revenue} presents the rolling average revenue earned per request. A higher per-request revenue indicates more SFCs accepted for longer durations.
...
The per-node tenancy distribution (Figures~\ref{fig:tenancy_bar}--\ref{fig:tenancy_cdf}) reveals that PPO concentrates placements on a smaller set of preferred nodes compared to Best-Fit, exhibiting a \textit{hot-spotting} pattern. This is a direct and honest consequence of the reward structure. The security cost heuristic $\rho_r$ is a mean over all placement nodes: $\rho_r \propto \frac{1}{|\Pi_r|}\sum_n q_n \cdot \text{SFC}(n)$. On a high-security node where $q_n \approx 0$, the product $q_n \cdot \text{SFC}(n)$ remains near zero regardless of how large $\text{SFC}(n)$ grows. The PPO policy therefore learns that repeatedly placing SFCs on high-security nodes incurs a negligible risk penalty, making those nodes persistently attractive. Best-Fit, by contrast, selects $\arg\min_n \rho_n^{\text{node}} = q_n \cdot \text{SFC}(n) \cdot T_s/\tau_{\max}$ at each step: as $\text{SFC}(n)$ accumulates on any node, that node's per-node score grows and previously under-used nodes become comparatively more attractive, producing a natural anti-concentration effect. The PPO reward does not encode this per-node SFC-count aversion directly — the mean over nodes smooths out individual hot-spots. In security terms, PPO's hot-spotting on high-security nodes is arguably rational: the affected nodes carry very low $q_n$, so their increased co-location does not substantially raise $\rho_r$. Nevertheless, concentration on a small subset of nodes increases blast radius in the event of a correlated failure, a risk not captured by the current security cost model. An explicit co-location penalty or a per-node SFC-count cap in the reward function would be a straightforward countermeasure.
