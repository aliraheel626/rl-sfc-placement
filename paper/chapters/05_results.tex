...
Figure~\ref{fig:revenue} presents the rolling average revenue earned per request and Figure~\ref{fig:lost_revenue} presents the rolling average lost revenue per request due to incidents. Together these metrics capture the economic impact of placement quality: a higher per-request revenue indicates more SFCs accepted for longer durations, while a lower per-request lost revenue indicates fewer incident-induced service disruptions.
...
Figure~\ref{fig:incidents_compare} shows the rolling average realized incidents per request. This metric captures the Monte Carlo outcome of the stochastic incident process and validates that the deterministic risk integral $\rho_r$ correlates with actual incident occurrences.
