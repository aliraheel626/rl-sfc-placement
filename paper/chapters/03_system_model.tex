...
\textbf{Incident downtime constraint:} If node $n_k$ is currently in a downtime period following a realized security incident (i.e.\ its remaining downtime counter $\Delta_{n_k}^{\text{down}}(t) > 0$), it is temporarily unavailable regardless of resources. Downtime counters decrement by one on each request tick and are set to $D_{\text{inc}} = 2$ steps upon an incident.
...
\end{equation}
where $R_{\text{acc}} = 1$ is the base acceptance reward, $\lambda = 4$ is the risk penalty coefficient, $\rho_r \in [0,1]$ is the risk integral for request $r$, $\mu = 0.05$ is the expected-loss reward scale, $\widehat{C}_r$ is the deterministic expected lost revenue (see Section~3.5.2), $\omega = 0.5$ is the windowed AR weight, and $\overline{\text{AR}}_w$ is the acceptance ratio over the last $w = 10$ requests. The use of expected lost revenue $\widehat{C}_r$ instead of stochastic realized cost reduces gradient variance, giving the policy a smoother learning signal.
...
Incident pressure models the temporal degradation of a node's security posture following a security event, and recovers geometrically as the node is remediated:
\begin{equation}
...
\textbf{Node downtime.} When a node experiences a realized incident, it enters a downtime period of $D_{\text{inc}} = 2$ request ticks, during which its resources are zeroed and no VNF may be placed on it. The remaining downtime counter $\Delta_n^{\text{down}}(t)$ decrements by one each tick. This models real-world remediation: an incident renders the node temporarily unavailable until service is restored.
...
For an accepted placement $\pi_r$ with TTL $\tau_r$, the security posture risk integral $\rho_r \in [0,1]$ quantifies the expected loss due to incident exposure across all placement nodes. Let $\Pi_r^{\text{dist}}$ denote the set of distinct nodes in $\pi_r$. The per-node base incident probability is:
...
The exposure window is $T_s = \min(\tau_r, T_{\text{cap}})$ with $T_{\text{cap}} = 12$. The risk integral is a deterministic expected-loss score:
...
\textbf{Stochastic incident simulation.} Although $\rho_r$ is deterministic, realized incidents are still simulated to drive the security posture dynamics. For each node $n \in \Pi_r^{\text{dist}}$, $T_s$ independent Bernoulli trials with success probability $q_n$ are drawn:
\begin{equation}
I_r = \sum_{n \in \Pi_r^{\text{dist}}} \sum_{t=1}^{T_s} \mathbf{1}\!\left[\text{Bernoulli}(q_n) = 1\right].
\end{equation}
Nodes that suffer at least one hit receive a pressure increment $\Delta_p$ and enter a downtime period of $D_{\text{inc}}$ request ticks.

\textbf{Incident cost (stochastic).} The realized incident cost is the revenue lost due to downtime across all affected nodes:
\begin{equation}
C_r = \sum_{n:\, \text{hit}} \text{SFC}(n) \cdot R_{\text{ttl}} \cdot D_{\text{inc}},
\end{equation}
where $R_{\text{ttl}}$ is the revenue per TTL step (default 1.0) and $D_{\text{inc}} = 2$ is the incident downtime in request ticks.

\textbf{Expected lost revenue (deterministic).} The noise-free counterpart used in the reward signal is:
\begin{equation}
\widehat{C}_r = \sum_{n \in \Pi_r^{\text{dist}}} P_n^{\text{hit}} \cdot \text{SFC}(n) \cdot R_{\text{ttl}} \cdot D_{\text{inc}},
\end{equation}
where $P_n^{\text{hit}} = 1 - (1 - q_n)^{T_s}$ is the probability that node $n$ suffers at least one incident during the exposure window. This provides a smooth gradient signal to the policy while remaining semantically aligned with the stochastic cost.
...
\end{equation}
with discount factor $\gamma = 0.99$. This objective simultaneously captures acceptance ratio (via the $R_{\text{acc}} / R_{\text{rej}}$ terms), latency efficiency (via intermediate latency penalties), security posture risk (via the $\lambda \rho_r$ penalty), and revenue protection (via the $\mu \widehat{C}_r$ expected-loss penalty), providing a holistic measure of intelligent, security-posture-aware placement quality.
