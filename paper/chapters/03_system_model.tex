...
\end{equation}
where $R_{\text{acc}} = 0.7$, $\alpha_s^T = 0.5$, $\alpha_c^T = 1.0$, $\alpha_{\text{sp}}^T = 0.3$ are the terminal shaping weights, $\text{SFC}^{-}(n_{K_r})$ is the chain count on the last placement node \emph{before} the current SFC is registered, $\omega = 0.5$, and $\overline{\text{AR}}_w$ is the acceptance ratio over the last $w = 10$ requests. $R_{\text{acc}}$ is set to 0.7 (rather than 1.0) to reduce the dominance of the acceptance signal relative to the quality shaping terms.
