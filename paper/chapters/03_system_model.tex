...
s_{n_k} \geq s_r^{\min},
\end{equation}
where $s_{n_k}$ is the static security score of node $n_k$.
...
\begin{enumerate}
  \item \textbf{Node feature matrix} $\mathbf{F} \in \mathbb{R}^{N \times 15}$: for each substrate node, a normalized vector comprising: (i)~base resources [RAM, CPU, storage, security score, average neighbor bandwidth, distance to previous placement node]; (ii)~global resource shares [RAM global share, CPU global share, storage global share] indicating each node's fraction of total consumed resources; (iii)~tenancy features [VNF global share, SFC tenancy ratio]; (iv)~VNF fit ratios [fit RAM, fit CPU, fit storage] measuring how much of each node's remaining capacity the current VNF would consume; (v)~load norm [VNF count relative to the 75th-percentile reference].
  \item \textbf{Global context vector} $\mathbf{g} \in \mathbb{R}^{17}$: encoding current VNF demands (3), SFC constraints (3), VNF progress index (1), hard isolation flag (1), latency budget consumed (1), TTL (1), remaining VNF count and aggregate demands (4), windowed acceptance ratio (1), episode running-average SFC tenancy (1), and episode running-average security margin (1). The last two entries give the agent a real-time view of how well it is managing co-location spread and security quality across the current episode, enabling adaptive mid-episode planning.
...
\end{equation}
The 0.1 progress bonus rewards feasible VNF placements; the latency term encourages locality. The security margin term normalises by the available range above the request minimum $s_r^{\min}$, giving a true $[0,1]$ signal regardless of the request's security requirement. The co-location penalty ($\alpha_c = 0.5$, primary) uses an inverse saturation $1 - 1/(1 + \text{SFC}(n_k))$, discouraging hot-spot nodes. The spread bonus ($\alpha_{\text{sp}} = 0.3$) provides an additional direct incentive for choosing a node with no existing SFC chains, creating a sharp gradient toward unoccupied nodes.
...
\end{equation}
with $\overline{\text{AR}}_w$ the windowed acceptance ratio ($\omega = 0.5$), $\bar{\rho}_t$ the episode running-average SFC tenancy normalised to $[0,1]$ ($\omega_c = 0.3$), and $\bar{\sigma}_t$ the episode running-average security margin normalised to $[0,1]$ ($\omega_s = 0.15$). High co-location reduces the per-SFC reward (analogous to how low AR increases the rejection penalty); high security margin amplifies it. $R_{\text{acc}} = 0.7$ is set below 1.0 to reduce acceptance dominance relative to the quality shaping terms.
