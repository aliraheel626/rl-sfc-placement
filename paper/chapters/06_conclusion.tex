...
\begin{enumerate}
  ...
  \item A \textbf{composite security posture and risk model} that quantifies operational risk as a normalized weighted sum of VNF tenancy, SFC tenancy, inverse security posture, stochastic incident dynamics with pressure accumulation and decay, and TTL-proportional exposure, providing a principled measure of placement-level security posture risk.
...
The security posture risk model introduces a principled operational objective beyond simple acceptance ratio maximization. By coupling security posture awareness to the reward signal, the intelligent DRL agent learns to prefer placements that are both feasible and operationally resilient, avoiding nodes with degraded security posture and reducing the long-term cost of security incidents and node failures. The stochastic incident process with pressure accumulation provides a realistic model of the temporal dynamics of node security posture in a shared NFV environment.
