...
\end{equation}
The 0.1 progress bonus rewards feasible VNF placements; the latency term encourages locality. The security margin term ($\alpha_s = 0.1$) rewards placing on nodes whose security score exceeds the request minimum, directly incentivising high-margin placements. The co-location penalty ($\alpha_c = 0.1$) uses an inverse saturation $1 - 1/(1 + \text{SFC}(n_k))$ that grows quickly for the first few chains and asymptotically approaches 1, discouraging hot-spot nodes without requiring a tunable reference parameter. With $\alpha_c = 0.5 > \alpha_s = 0.3$, co-location spread is the primary per-step objective; larger terminal weights $\alpha_c^T = 1.0$ and $\alpha_s^T = 0.5$ provide a stronger quality signal at SFC completion.
...
\end{equation}
where $R_{\text{acc}} = 1$, $\alpha_s^T = 0.5$ and $\alpha_c^T = 1.0$ are the terminal shaping weights (larger than the intermediate $\alpha_s = 0.3$, $\alpha_c = 0.5$ to provide a stronger end-of-SFC quality signal; $\alpha_c^T > \alpha_s^T$ reflects the priority of co-location spread), $\text{SFC}^{-}(n_{K_r})$ is the chain count on the last placement node \emph{before} the current SFC is registered, $\omega = 0.5$, and $\overline{\text{AR}}_w$ is the acceptance ratio over the last $w = 10$ requests.
