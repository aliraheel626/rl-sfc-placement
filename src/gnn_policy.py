...
class GNNFeaturesExtractor(BaseFeaturesExtractor):
    ...
    def __init__(
        ...
        Args:
            ...
            node_feat_dim: Dimension of per-node features (default: 15)
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        Args:
            observations: Dict with keys:
                - node_features:  (batch, max_nodes, 15)
        ...
        node_feats = observations["node_features"]  # (B, M, 15)
        ...
        node_feats_real = node_feats[:, :N_real, :]  # (B, N_real, 15)
        placement_real = placement_mask[:, :N_real].unsqueeze(2)  # (B, N_real, 1)

        # Combine node features with placement mask → (B, N_real, 16)
