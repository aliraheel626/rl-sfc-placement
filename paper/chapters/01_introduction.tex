...
placement work, the framework models security posture along three
independent CIA (Confidentiality, Integrity, Availability) dimensions,
incorporates trust-zone affinity and path-level link security as
explicit hard constraints, and shapes the reward signal to incentivise
placements with security posture that exceeds the required minimum.
The agent is a Maskable Proximal Policy
Optimization (Maskable PPO) learner whose action mask enforces every
hard constraint --- resource, latency, bandwidth, CIA security,
trust zone, link security, and isolation --- at both training and
inference time, so that every chain the agent accepts is feasible by
construction. A Graph Neural Network
...
\begin{enumerate}
  ...
  \item Develop a \textbf{multi-dimensional CIA triad security model}
  that represents confidentiality, integrity, and availability as
  independent node attributes, enforces them as hard placement
  constraints, and shapes the reward signal to incentivise placements
  with security posture exceeding the hard-constrained minimum.
  ...
  latency, bandwidth, CIA security, trust-zone, link-security, and
  isolation requirements by construction.
  ...
  best-fit baseline enforcing identical hard constraints, aligning the
  random-number stream across algorithms so that both see identical
  request sequences.
\end{enumerate}
...
\begin{enumerate}
  ...
  bandwidth, CIA security, trust-zone, link-security, and isolation
  requirements are all enforced before any action is sampled --- so
  every chain the agent accepts is feasible by construction.
  \item A \textbf{multi-dimensional CIA triad security model} that
  independently represents and enforces confidentiality, integrity,
  and availability as per-node security attributes, supplemented by
  trust-zone affinity constraints and minimum path-level link security
  requirements --- providing a richer and more operationally realistic
  security model than prior scalar security-score approaches.
  \item A \textbf{security-aware reward signal} combining per-VNF CIA
  margin bonuses, per-hop link security rewards, and an SFC-level CIA
  acceptance bonus, giving the DRL agent an explicit incentive to
  prefer placements with security posture exceeding the hard-constrained
  minimum.
  ...
  Best-Fit baseline so that both process identical request streams,
  with evaluation covering acceptance ratio, CIA security margins,
  zone compliance, link security margin, and substrate utilization.
\end{enumerate}
...
CIA triad security model with trust zones and link security. Chapter~4
presents the proposed methodology in detail, including the Maskable PPO
agent, the GNN feature extractor, the security-aware reward design, the
action-masking implementation, and the best-fit baseline. Chapter~5
describes the experimental setup and presents comparative results, and
