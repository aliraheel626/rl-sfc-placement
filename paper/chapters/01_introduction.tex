...
placement work, the framework treats security posture as a dynamic,
load- and history-dependent state of each substrate node, integrates it
directly into both the constraint mask and the reward signal, and
evaluates placement decisions against an expected-loss risk model
...
Despite growing interest in DRL for SFC placement, four gaps persist in
the literature reviewed in Chapter~2:

\begin{enumerate}
  \item Existing DRL placement agents treat constraints as soft
  penalties, allowing infeasible actions during training and inference;
  hard-constraint guarantees are the exception, not the rule.
  \item Security is typically modelled statically: node scores are
  checked against thresholds but no dynamic posture, no incident
  history, and no load-dependent incident probability are maintained
  --- in direct contrast to the posture-based view advocated by the
  security-metrics and 5G-security
  communities~\cite{pendleton_survey_2017, abdelrazek_measuring_2025, editor_security_nodate}.
  \item Policies are usually topology-blind: they consume flattened
  feature vectors and fail to exploit the graph structure of the
  substrate.
  \item Comparative evaluations rarely equalize the stream of requests
  seen by different algorithms, so apparent performance gaps may
  instead reflect stream variance.
\end{enumerate}

The objectives of this thesis follow directly from these gaps:

\begin{enumerate}
  \item Design a DRL framework for SFC placement in which every
  hard constraint --- including a security-posture constraint ---
  is enforced by action masking, guaranteeing feasibility of every
  accepted placement.
  \item Develop an expected-loss security posture risk model that
  quantifies operational risk as a function of per-node incident
  ...
  \item Integrate a Graph Neural Network feature extractor into the
  policy so that placement decisions reflect substrate topology and
  the structural role of each node.
  \item Perform a fair comparative evaluation against a
...
\begin{enumerate}
  ...
  \item An \textbf{expected-loss security posture risk model}
  combining load-dependent incident probability, chain co-location
  (blast radius), and TTL-proportional exposure into a deterministic
  risk integral $\rho \in [0,1]$, paired with a deterministic expected
  lost revenue term that provides a low-variance gradient signal for
  policy optimization.
  ...
  acceptance ratio, risk integral, security margin, substrate
  utilization, per-node SFC/VNF tenancy, total and per-request
  revenue, expected and realized lost revenue, and realized incident
  counts.
\end{enumerate}
