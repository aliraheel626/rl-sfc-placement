...
Industry tooling operationalizes this idea through composite scoring
systems: Microsoft Secure Score, for instance, continuously evaluates
an organization's posture across identities, endpoints, applications,
and data against NIST and CIS benchmarks, expressing the result as a
normalized scalar that rises or falls as controls are applied or
removed~\cite{arvatosystems2025securescore}. This thesis adopts the
same abstraction at the node level: each substrate node carries a
discrete security score $s_n \in [1, 5]$ that summarizes its current
posture, and SFC requests declare a minimum acceptable score
$s_r^{\min}$ that every placement node must satisfy.
...
\begin{enumerate}
  ...
  \item Design a \textbf{security-aware reward shaping} scheme that
  applies a per-VNF security margin bonus and co-location penalty at
  every placement step, steering the DRL policy toward high-security,
  low-blast-radius placements without a separate risk model.
  ...
  Best-Fit by Maximum Security Margin baseline, aligning the random-number
...
\begin{enumerate}
  ...
  \item A \textbf{security-aware reward shaping} scheme applying a
  per-VNF security margin bonus ($\alpha_s$) and co-location penalty
  via inverse saturation ($\alpha_c$) at every placement step,
  steering the DRL policy toward high-security, low-blast-radius
  placements without a separate risk model.
  ...
  Best-Fit by Maximum Security Margin baseline so that both process identical
  request streams.
...
security-aware reward shaping. Chapter~4 presents the proposed
