...
\begin{enumerate}
  ...
  \item Develop an expected-loss security posture risk model that quantifies operational risk as a function of per-node incident probability, chain co-location (blast radius), and time-exposure, together with a deterministic expected lost revenue signal for smooth policy optimization, and incorporate both into the DRL reward signal.
  ...
  \item Evaluate the proposed framework against a risk-minimizing Best-Fit heuristic on comprehensive metrics including acceptance ratio, risk integral, security margin, substrate utilization, tenancy, revenue, expected lost revenue, and realized incident rates.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{An expected-loss security posture risk model} that computes the risk integral $\rho \in [0,1]$ as the mean per-node product of incident probability (driven by security posture, load, and incident pressure), chain co-location count, and TTL-proportional exposure, paired with a deterministic expected lost revenue penalty that provides low-variance gradient signal for policy optimization.
  ...
  \item \textbf{Comprehensive experimental analysis} across acceptance ratio, risk integral, security margin, substrate utilization, SFC/VNF tenancy, total revenue, expected lost revenue, and realized incident rates.
\end{enumerate}
